 ...
 * - Add To Cart -> localStorage("knx_cart") + DB sync (knx_carts / knx_cart_items)
 * ==========================================================
 */
(function () {
  'use strict';
  ...
  function initFilter(root) {
    ...
    // Two search inputs (mobile + desktop)
    ...
    function syncAndFilter(sourceInput) {
      ...
      // Keep both inputs in sync visually
  ...
  function initModal(root) {
    ...
    cards.forEach(function (card) {
      card.addEventListener('click', function () {
        ...
        openModal();
        attachModalLogic(modalBody, price, modifiers, meta, closeModal);
      });
    });
  }
  ...
  /* ==========================================================
   * MODAL LOGIC + ADD TO CART
   * ========================================================== */
  function attachModalLogic(container, basePrice, modifiers, meta, closeModalFn) {
    var qty = 1;
    var selectedSingles = {};
    ...
    addBtn.addEventListener('click', function () {
      ...
      var cart = readCart();
      cart.push(cartItem);
      saveCart(cart);       // localStorage + knx-cart-updated
      syncCartServer(cart); // DB sync
  ...
  /**
   * Get or create persistent cart token stored in cookie.
   */
  ...
  /**
   * Sync current cart (localStorage mirror) to DB via REST API.
   * Writes into knx_carts + knx_cart_items.
   */
  function syncCartServer(cartArray) {
    try {
      ...
      fetch('/wp-json/knx/v1/cart/sync', {
        ...
        // Do not break UI if sync fails
      });
    } catch (e) {
      // Silent fail, UI should still work with localStorage
    }
  }
