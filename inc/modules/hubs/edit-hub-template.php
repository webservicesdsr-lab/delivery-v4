...
add_shortcode('knx_edit_hub', function () {
    ...
    <div class="knx-edit-hub-wrapper"
      ...
      <div class="knx-card" id="locationBlock">
        ...
        <?php if (empty($maps_key)) : ?>
          <div class="knx-info-banner" style="background:#e3f2fd;border-left:4px solid #2196f3;padding:12px;margin-bottom:16px;border-radius:6px;">
            <strong>ℹ️ Using OpenStreetMap</strong><br>
            <small>Google Maps API key not configured. You can add one in
              <a href="<?php echo esc_url(admin_url('admin.php?page=nexus-settings')); ?>">Nexus Settings</a>
              to enable Google Places Autocomplete.</small>
          </div>
        <?php endif; ?>

        <div class="knx-form-group">
          <label>Address <?php if (empty($maps_key)) echo '<small>(Type and press Enter)</small>'; ?></label>
          <input type="text"
                 id="hubAddress"
                 list="addrList"
                 value="<?php echo esc_attr($hub->address ?? ''); ?>"
                 placeholder="<?php echo empty($maps_key) ? 'Type address and press Enter…' : 'Search address…'; ?>">
        ...
        <div style="margin: 20px 0; padding: 16px; background: #f8fafc; border-radius: 8px; border: 1px solid #e5e7eb;">
          <label style="display:block; font-weight:600; margin-bottom:14px; color:#111; font-size:15px;">
            🚚 Delivery Zone Type
          </label>
          ...
          <div style="display: flex; gap: 24px; margin-bottom: 16px;">
            ...
            <label style="display: flex; align-items: center; cursor: pointer;">
              <input type="radio" name="deliveryZoneType" value="radius" 
                     <?php echo (($hub->delivery_zone_type ?? 'radius') === 'radius') ? 'checked' : ''; ?>
                     style="margin-right: 8px; cursor: pointer; width: 18px; height: 18px;" />
              <span style="font-weight: 500; font-size: 14px;">Radius (Simple Circle)</span>
            </label>
            
            <!-- Polygon Option -->
            <label style="display: flex; align-items: center; cursor: pointer;">
              <input type="radio" name="deliveryZoneType" value="polygon"
                     <?php echo (($hub->delivery_zone_type ?? 'radius') === 'polygon') ? 'checked' : ''; ?>
                     style="margin-right: 8px; cursor: pointer; width: 18px; height: 18px;" />
              <span style="font-weight: 500; font-size: 14px;">Custom Polygon (Draw Area)</span>
            </label>
          </div>

          <!-- Radius Options (shown by default) -->
          <div id="radiusOptions" style="display: <?php echo (($hub->delivery_zone_type ?? 'radius') === 'radius') ? 'block' : 'none'; ?>;">
            <label for="deliveryRadius" style="display:block; font-weight:500; margin-bottom:6px; color:#374151; font-size:14px;">
              Delivery Radius (miles)
            </label>
            <input type="number" id="deliveryRadius" step="0.1" min="0"
                   value="<?php echo esc_attr($hub->delivery_radius ?? '3'); ?>"
                   style="width:150px; padding:10px; border:1px solid #ddd; border-radius:6px; font-size:14px;">
          ...
          <div id="polygonOptions" style="display: <?php echo (($hub->delivery_zone_type ?? 'radius') === 'polygon') ? 'block' : 'none'; ?>;">
            <div style="background: #fff; padding: 14px; border-radius: 6px; border: 1px solid #e5e7eb; margin-bottom: 14px;">
            ...
            <div style="display: flex; gap: 10px; flex-wrap: wrap; margin-bottom: 14px;">
              <button type="button" id="startDrawing" 
                style="padding: 11px 20px; background: #0b793a; color: white; border: none; border-radius: 6px; cursor: pointer; font-weight: 500; font-size: 14px; transition: background 0.2s;">
                🖊️ Start Drawing
              </button>
              <button type="button" id="completePolygon" disabled
                style="padding: 11px 20px; background: #cbd5e1; color: #64748b; border: none; border-radius: 6px; cursor: not-allowed; font-weight: 500; font-size: 14px;">
                ✅ Complete Polygon
              </button>
              <button type="button" id="clearPolygon" disabled
                style="padding: 11px 20px; background: #f1f5f9; color: #94a3b8; border: none; border-radius: 6px; cursor: not-allowed; font-weight: 500; font-size: 14px;">
                🗑️ Clear
              </button>
            </div>

            <div id="polygonStatus" style="padding: 12px; background: #f3f4f6; border-radius: 6px; font-size: 14px; color: #6b7280;">
              Click "Start Drawing" to begin
            </div>
          </div>
        </div>

        <input type="hidden" id="hubLat" value="<?php echo esc_attr($hub->lat ?? '41.12000000'); ?>">
        <input type="hidden" id="hubLng" value="<?php echo esc_attr($hub->lng ?? '-87.86110000'); ?>">
      ...
      <div class="knx-card knx-danger-card" id="deleteHubBlock">
        ...
        <div class="knx-danger-content">
          ...
          <button id="deleteHubBtn" class="knx-btn knx-btn-danger" data-hub-id="<?php echo esc_attr($hub_id); ?>" data-hub-name="<?php echo esc_attr($hub->name); ?>">
    ...
    <!-- Dual Map (Google ↔ Leaflet) + Nominatim/datalist -->
