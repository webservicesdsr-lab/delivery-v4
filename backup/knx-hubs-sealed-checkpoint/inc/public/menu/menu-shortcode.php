...
function knx_render_menu_page() {
	...
	/* ==========================================================
	   STATUS LOGIC (open/closed)
	========================================================== */
	$now_ts      = current_time('timestamp');
	$status_text = 'Closed';

	$is_temp_closed =
		!empty($restaurant->is_temp_closed) &&
		!empty($restaurant->closure_until) &&
		strtotime($restaurant->closure_until) > $now_ts;

	if ($is_temp_closed) {
		$reopen_ts = strtotime($restaurant->closure_until);
		$status_text = $reopen_ts
			? 'Closed (reopens ' . date_i18n('M j, g:i A', $reopen_ts) . ')'
			: 'Temporarily closed';
	} else {
...
<div class="knx-mobile-search">
	...
	<div class="knx-menu__modal" id="knxMenuModal" style="display:none;">
		...
		<div class="knx-menu__modal-dialog">
			...
			<!-- CLOSE WRAPPER (sticky container) -->
