...
/**
 * Kingdom Nexus - Explore Hubs API (v1.1 Clean & Optimized)
 * ----------------------------------------------------------
 * - GET /knx/v1/explore-hubs
 * - GET /knx/v1/explore-hubs-spotlights
 * - GET /knx/v1/explore-hubs-cuisines
 * ----------------------------------------------------------
 * This version preserves ALL existing logic while fixing:
 * ✔ category_id not being returned
 * ✔ category_name always null
 * ✔ $wpdb not available inside closure
...
function knx_api_explore_hubs(WP_REST_Request $r) {
    ...
    $sql = "
        SELECT
            ...
            h.hours_saturday,
            h.hours_sunday,
            h.category_id,              -- FIXED: category support
            c.name AS city
        FROM {$table_hubs} h
